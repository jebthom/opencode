...
import { CodeGraphDeterministic } from "@/codegraph/deterministic"
import { RECENCY_TAG_IDS } from "@/codegraph/collections"

const file = (id: string, path: string, mtime = 0, size = 10): CodeGraphDeterministic.SubtreeFile => ({
  id,
  path,
  size,
  mtime,
...
describe("deterministic git-changed", () => {
  ...
  test("tags only files in the working-tree change set as changed, bridging the repo prefix", () => {
    ...
    const store = CodeGraphDeterministic.computeStore("git-changed", subtree, {
      prefix: "pkg/app/",
      changed: new Set(["pkg/app/src/a.ts", "pkg/app/src/c.ts"]),
    })
    expect(store["n1"]!.tag).toBe("changed")
    expect(store["n2"]!.tag).toBe("unchanged")
    expect(store["n3"]!.tag).toBe("changed")
  })

  test("no prefix (repo root) and empty/absent change set", () => {
    const atRoot = CodeGraphDeterministic.computeStore("git-changed", subtree, {
      prefix: "",
      changed: new Set(["src/b.ts"]),
    })
    expect(atRoot["n2"]!.tag).toBe("changed")
    expect(atRoot["n1"]!.tag).toBe("unchanged")
    // Missing git info → everything unchanged rather than throwing.
    const none = CodeGraphDeterministic.computeStore("git-changed", subtree, undefined)
    expect(Object.values(none).every((e) => e.tag === "unchanged")).toBe(true)
  })
})
...
describe("deterministic mtime buckets", () => {
  test("equal time spans: a checkout-time majority shares bucket 0, recent edits climb to the warm end", () => {
    ...
    const store = CodeGraphDeterministic.computeStore("mtime-buckets", subtree, undefined)
    // The clustered majority all land in the oldest bucket — "vast majority one colour".
    for (const id of ["a", "b", "c", "d", "e"]) expect(store[id]!.tag).toBe(RECENCY_TAG_IDS[0])
    // The single recent edit clamps into the newest (warm) bucket.
    expect(store["z"]!.tag).toBe(RECENCY_TAG_IDS[5])
  })
  ...
  test("all-equal mtime collapses to one colour; missing mtime (0) is treated as oldest", () => {
    ...
    const store = CodeGraphDeterministic.computeStore("mtime-buckets", equal, undefined)
    expect(Object.values(store).every((e) => e.tag === RECENCY_TAG_IDS[0])).toBe(true)
    ...
    const s2 = CodeGraphDeterministic.computeStore("mtime-buckets", withZero, undefined)
    expect(s2["zero"]!.tag).toBe(RECENCY_TAG_IDS[0]) // clamped to the oldest real mtime
    expect(s2["old"]!.tag).toBe(RECENCY_TAG_IDS[0])
    expect(s2["new"]!.tag).toBe(RECENCY_TAG_IDS[5])
  })
})
